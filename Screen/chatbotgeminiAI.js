...
const ChatbotGeminiAI = () => {
  ...
  const handleButtonClick = async () => {
    ...
    try {
      ...
      setMessages(prevMessages => [
        {text: 'Error occurred while fetching data', sender: 'gemini'},
        ...prevMessages,
      ]);
      setTyping(false);
    }
  ...
  const simulateTyping = reply => {
    ...
    const interval = setInterval(() => {
      if (currentText.length < reply.length) {
        ...
        setMessages(prevMessages => [
          {text: currentText, sender: 'gemini'},
    ...
    setMessages(prevMessages => [
      {text: '', sender: 'gemini'}, // Initial empty "typing" message
      ...prevMessages,
    ]);
  };
